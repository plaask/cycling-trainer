...
- **已安装 Temurin JDK 21** 到：`D:\coding\Android\jdk-21\jdk-21.0.12.1+1`
- 命令行构建必须设：
  ```powershell
  $env:JAVA_HOME='D:\coding\Android\jdk-21\jdk-21.0.12.1+1'
  $env:Path="$env:JAVA_HOME\bin;$env:Path"
  $env:GRADLE_USER_HOME='D:\coding\cycling-trainer\.gradle-home'   # 沙箱内固定
  $env:ANDROID_USER_HOME='D:\coding\cycling-trainer\.android'      # debug keystore 位置
  $env:TMP='D:\coding\cycling-trainer\.tmp'; $env:TEMP=$env:TMP    # 关键！沙箱 TEMP 每次不同导致 native dll 解压失败
  ```

## 2. Gradle wrapper

- 已生成：`gradlew.bat` + `gradle/wrapper/gradle-wrapper.jar`（8.10.2）。
- 本地发行版 `C:\Users\11988\AppData\Local\GradleDist\gradle-8.10.2\bin\gradle.bat` 可直接用。

## 3. Android SDK platform（关键坑）

- SDK 里 platform 目录叫 **`android-37.0`**（Google 2026 打包，Android 17），AGP 8.7.3 需要 `platforms;android-37`。
- 处理（两步）：
  1. 在 `D:\coding\Android\Sdk\platforms` 建 junction：`android-37 -> android-37.0`
...
- AGP 每次尝试下载 SDK 清单到 `C:\Users\11988\.android\cache` 会因权限失败——不影响构建，只是日志噪音。
